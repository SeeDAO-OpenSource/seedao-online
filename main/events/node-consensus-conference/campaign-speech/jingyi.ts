...
export default class NCCLobbyNPC1Event extends RpgEvent {
    onInit() {
        this.setGraphic('jingyi')
        this.setComponentsTop(Components.text('JingYi', {
            fill: '#ffffff',
            fontSize: 12,
            fontFamily: 'Arial',
            stroke: '#000000',
    ...
    async onAction(player: RpgPlayer) {
        ...
        const voteChoice = await player.showChoices('你会投票支持我(JingYi)吗？', [
            { text: '支持！', value: 'yes' },
        ...
        if (voteChoice && voteChoice.value === 'yes') {
            ...
            await player.showText('没关系，欢迎随时找我聊聊，我很乐意分享我的想法！', {
                talkWith: this
            })
